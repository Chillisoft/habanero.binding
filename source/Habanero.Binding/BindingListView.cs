        ...
        where T : class, IBusinessObject, new()
    {
        ...
        private void InitializeFiltering()
        {
            ...
            foreach (PropertyDescriptor property in _descriptorCollection)
            {
                ...
                colType = GetUnderlyingType(colType);
                _filterTable.Columns.Add(property.Name, colType);
            }
        }
        /// <summary>
        /// For Nullable Types get the underlying column Type.
        /// </summary>
        /// <param name="colType"></param>
        /// <returns></returns>
        private Type GetUnderlyingType(Type colType)
        {
            if (IsNulllable(colType)) colType = colType.GetGenericArguments()[0];
            return colType;
        }

        private bool IsNulllable(Type colType)
        {
            return colType.IsGenericType && colType.GetGenericTypeDefinition().Equals(typeof (Nullable<>));
        }
        ...
        public void CopyTo(Array array, int index)
        {
            ...
            if (index < 0) throw new ArgumentOutOfRangeException("index", "Number was less than the array's lower bound in the first dimension");
            if (index >= array.Length) throw new ArgumentException("Destination array was not long enough. Check destIndex and length, and the array's lower bounds", "index");
            if (BusinessObjectCollection != null) BusinessObjectCollection.CopyTo(array, index);
        }

        public int Count
        {
            get { return this.BusinessObjectCollection.Count; }
        }
...
/*                    _sortPropertyValue = property;
            ...
            set
            {
                ...
                else
                {
                    ...
                    while (count < matches.Length)
                    {
                        ...
                        if (!String.IsNullOrEmpty(_filterValue) && !value.Contains(_filterValue)) ResetList();
        ...
        private void ApplyFilter(SingleFilterInfo filterParts)
        {
            ...
            BusinessObjectCollection.Clear();
            foreach (T itemFound in results) BusinessObjectCollection.Add(itemFound);
        }
...
//                Items.Add(t);
            ...
            //TODO brett 22 Nov 2010: 
/*            if (IsSortedCore)
                if (SortPropertyCore != null) ApplySortCore(SortPropertyCore, SortDirectionCore);*/
        }
        ...
        {
            return this.BusinessObjectCollection.GetEnumerator();
        }
    }
/*
